...
Sync::Sync(UdpMsg::connect_status* connect_status) : _config(), _rollingback(false),
                                                     _input_queues(nullptr),
...
Sync::~Sync()
{
	 ...
	 */
	for (int i = 0; i < std::size(_savedstate.frames); i++)
	{
...
{
	const int frames_behind = _framecount - _last_confirmed_frame;
...
void Sync::AddRemoteInput(int queue, GameInput& input) const
{
	_input_queues[queue].AddInput(input);
}

int Sync::GetConfirmedInputs(void* values, int size, int frame) const
{
...
int Sync::SynchronizeInputs(void* values, int size) const
{
	int disconnect_flags = 0;
	char* output = (char*)values;
...
void Sync::AdjustSimulation(int seek_to)
{
	...
	const int count = _framecount - seek_to;
...
int Sync::FindSavedFrameIndex(int frame) const
{
...
bool Sync::CheckSimulationConsistency(int* seekTo) const
{
	int first_incorrect = GameInput::NullFrame;
	for (int i = 0; i < _config.num_players; i++)
	{
		const int incorrect = _input_queues[i].GetFirstIncorrectFrame();
		Log("considering incorrect frame %d reported by queue %d.\n", incorrect, i);

		if (incorrect != GameInput::NullFrame && (first_incorrect == GameInput::NullFrame || incorrect <
			first_incorrect))
		{
...
void Sync::SetFrameDelay(int queue, int delay) const
{
	_input_queues[queue].SetFrameDelay(delay);
}


void Sync::ResetPrediction(int frameNumber) const
{
	for (int i = 0; i < _config.num_players; i++)
	{
